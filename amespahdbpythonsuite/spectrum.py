        ...
        **keywords,
        ...
        for uid, spec in self.data.items():
            ...
            if normalization == 'max':
                max_val = np.max(s)
                s /= max_val if max_val != 0 else 1
            elif normalization == 'area':
                area = np.sum(s)
                s /= area if area != 0 else 1
            matrix.append(s)
        ...
        **keywords,
        ...
        if multiprocessing:
            ...
            for solution, b in tqdm(
                ...
                for uid, s, m_element in zip(self.uids, solution, matrix):
                    if s > 0:
                        uids.append(uid)
            ...
            for _ in tqdm(
                ...
                # Fit the spectrum.
                fit = self.fit(flux * obs.flux.unit, 
                               obs.uncertainty, 
                               notice=False, 
                ...
                if fit:
                    ...
                    dof = len(obs.flux) - len(fit.data)
                    fit.rchi2 = chi2 / dof if dof > 0 else float("inf")
                    mcfits.append(fit)
